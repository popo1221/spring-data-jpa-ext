...
public class NotLikeSpecification<T, P> extends AbstractSpecification<T> {
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
        ...
        if (this.pattern instanceof Expression) {
            return criteriaBuilder.notLike(xExpression,
                    ((ExpressionHolder<String>) pattern).getExpression(root, criteriaQuery, criteriaBuilder));
        }

        return criteriaBuilder.notLike(xExpression, (String) pattern);
    }
}
